...
const FICHES_DATABASE = {
  societes: [
    { id: 'soc_11', title: 'Constitution', file: 'Droit des sociétés fiches de révision/Fiche_Constitution.html' },
    { id: 'soc_13', title: 'Fin de l\'entreprise', file: 'Droit des sociétés fiches de révision/Fiche_Fin_Entreprise.html' },
    { id: 'soc_2', title: 'Financement', file: 'Droit des sociétés fiches de révision/Fiche_Financement.html' },
    { id: 'soc_3', title: 'Gouvernance', file: 'Droit des sociétés fiches de révision/Fiche_Gouvernement_Entreprise.html' },
    { id: 'soc_4', title: 'Groupe', file: 'Droit des sociétés fiches de révision/Fiche_Regime_Groupe.html' },
    { id: 'soc_14', title: 'Groupement sans but lucratif', file: 'Droit des sociétés fiches de révision/Fiche_Groupement_Non_Lucratif.html' },
    { id: 'soc_5', title: 'Restructuration', file: 'Droit des sociétés fiches de révision/Fiche_Restructurations.html' },
    { id: 'soc_6', title: 'Transformation', file: 'Droit des sociétés fiches de révision/Fiche_Transformation.html' },
    { id: 'soc_7', title: 'Transmission à titre gratuit', file: 'Droit des sociétés fiches de révision/Fiche_Transmission_Gratuit.html' },
    { id: 'soc_12', title: 'Transmission à titre onéreux', file: 'Droit des sociétés fiches de révision/Fiche_Transmission_Onereux.html' },
    { id: 'soc_8', title: 'Rappels DCG : SNC et SARL', file: 'Droit des sociétés fiches de révision/Fiche_SNC_SARL.html' },
    { id: 'soc_9', title: 'Rappels DCG : SA', file: 'Droit des sociétés fiches de révision/Fiche_SA_Gouvernance.html' },
    { id: 'soc_10', title: 'Rappels DCG : SAS, SCA et SC', file: 'Droit des sociétés fiches de révision/Fiche_SAS_SCA_SC.html' }
  ],
  fiscal: [
...
const QUIZ_DATABASE = [
  ...
  },
  {
    ...
    a: "Oui ! Même exacte, une information péjorative divulguée publiquement pour jeter le discrédit sur un concurrent et détourner sa clientèle constitue une faute de dénigrement."
  },
  {
    q: "En cas de rupture d'une relation commerciale établie, quelle durée de préavis met l'auteur de la rupture à l'abri d'une action pour durée insuffisante ?",
    a: "Un préavis écrit d'au moins 18 mois : depuis l'ordonnance du 24 avril 2019, la responsabilité ne peut plus être engagée du chef d'une durée insuffisante dès lors que ce préavis a été respecté (Art. L. 442-1 II C. com.)."
  },
  {
...
// Keep the homepage counters in sync with the real content.
// Total fiches is derived from the registry; pièges / cas counts are injected
// at bundle time as FICHE_STATS (falls back to the static HTML otherwise).
function updateHomeStats() {
  const totalFiches = Object.keys(FICHES_DATABASE)
    .reduce((n, s) => n + FICHES_DATABASE[s].length, 0);
  const setTxt = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
  setTxt('stat-total-fiches', totalFiches);
  if (typeof FICHE_STATS !== 'undefined') {
    setTxt('stat-total-pieges', FICHE_STATS.pieges);
    setTxt('stat-total-cas', FICHE_STATS.cas);
  }
}
...
function toggleTheme() {
  ...
  initTheme();
  // Propagate to open fiches (only available in the bundled build).
  if (typeof applyThemeToFiches === 'function') applyThemeToFiches(currentTheme);
}
...
function renderSidebars() {
  ['societes', 'fiscal', 'contrats', 'difficultes_penal', 'finance', 'evaluation', 'mcg'].forEach(subject => {
    ...
    // Empty subject (e.g. MCG not yet populated): show a friendly placeholder
    // instead of an empty column and a blank viewer.
    if (FICHES_DATABASE[subject].length === 0) {
      container.innerHTML = `<div class="sidebar-title">Fiches de Révision (0)</div>
        <div style="padding:16px; color:var(--text-muted); font-size:13.5px; line-height:1.5;">
          📦 Aucune fiche pour l'instant.<br>Cette section sera bientôt disponible.
        </div>`;
      const iframe = document.getElementById(`viewer-${subject}`);
      if (iframe) {
        iframe.removeAttribute('src');
        iframe.srcdoc = `<!doctype html><html lang="fr"><head><meta charset="utf-8">
          <style>body{margin:0;font-family:system-ui,Segoe UI,Arial,sans-serif;display:flex;
          align-items:center;justify-content:center;height:100vh;color:#64748b;background:#f8fafc;text-align:center}
          div{max-width:420px;padding:24px}h2{color:#1F4E79;margin:0 0 8px}</style></head>
          <body><div><div style="font-size:44px">🚧</div><h2>Bientôt disponible</h2>
          <p>Les fiches « Management et Contrôle de Gestion » seront ajoutées prochainement.</p></div></body></html>`;
      }
      return;
    }
